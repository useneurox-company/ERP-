...
interface BoardCardChecklistSectionProps {
  cardId: string;
  users: User[];
}

export function BoardCardChecklistSection({ cardId, users }: BoardCardChecklistSectionProps) {
  ...
  const { data: checklists = [], isLoading } = useQuery<Checklist[]>({
    queryKey: [`/api/boards/cards/${cardId}/checklists`],
    enabled: !!cardId,
  });
  ...
  const createChecklistMutation = useMutation({
    mutationFn: async (name: string) => {
      return apiRequest("POST", `/api/boards/cards/${cardId}/checklists`, { name });
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: [`/api/boards/cards/${cardId}/checklists`] });
      setNewChecklistName("");
  ...
  const updateChecklistMutation = useMutation({
    mutationFn: async ({ checklistId, data }: { checklistId: string; data: Partial<Checklist> }) => {
      return apiRequest("PUT", `/api/boards/cards/${cardId}/checklists/${checklistId}`, data);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: [`/api/boards/cards/${cardId}/checklists`] });
    },
  ...
  const deleteChecklistMutation = useMutation({
    mutationFn: async (checklistId: string) => {
      return apiRequest("DELETE", `/api/boards/cards/${cardId}/checklists/${checklistId}`);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: [`/api/boards/cards/${cardId}/checklists`] });
  ...
  const createItemMutation = useMutation({
    mutationFn: async ({ checklistId, item_text }: { checklistId: string; item_text: string }) => {
      return apiRequest("POST", `/api/boards/cards/${cardId}/checklists/${checklistId}/items`, { item_text });
    },
  ...
  const updateItemMutation = useMutation({
    mutationFn: async ({
      ...
      return apiRequest("PUT", `/api/boards/cards/${cardId}/checklists/${checklistId}/items/${itemId}`, data);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: [`/api/boards/cards/${cardId}/checklists`] });
      setEditingItem(null);
  ...
  const deleteItemMutation = useMutation({
    mutationFn: async ({ checklistId, itemId }: { checklistId: string; itemId: string }) => {
      return apiRequest("DELETE", `/api/boards/cards/${cardId}/checklists/${checklistId}/items/${itemId}`);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: [`/api/boards/cards/${cardId}/checklists`] });
    },
      ...
      {showAddChecklist && (
        <div className="mb-4 p-3 bg-muted rounded-md">
          ...
          <div className="flex gap-2 mt-2">
            <Button type="button" size="sm" onClick={handleCreateChecklist} disabled={!newChecklistName.trim()}>
              Создать
            </Button>
            <Button type="button" size="sm" variant="ghost" onClick={() => setShowAddChecklist(false)}>
              Отмена
            </Button>
          </div>
        </div>
